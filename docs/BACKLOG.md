...
**G7 discovery/status:** The n8n database contains active workflow records for `STI Risk - Ask Steve Staff Agent` and `STI Risk - Gemini RAG Reindex`; the staff workflow is configured to call `STI_STAFF_AGENT_CONTEXT_URL` plus `STI_RAG_SEARCH_URL`, and the repo-side internal context/RAG endpoints perform full-text/vector retrieval. However, the live n8n webhook probes returned `404 webhook not registered` for both the staff-agent and reindex production paths, so the external answer path and vectorization worker are not currently live-verified. The repo previously had no `help_guide` documents; commit `be5f70d` now seeds 14 Help Center guide/reference documents on production startup. Live DB verification shows all 14 are present but pending vectorization. Permission-boundary review remains outstanding before claiming unrestricted staff-wide retrieval.
