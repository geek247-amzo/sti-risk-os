...
**G7 discovery/status:** The active n8n workflow `STI Risk - Ask Steve Staff Agent` is present and calls `STI_STAFF_AGENT_CONTEXT_URL` plus `STI_RAG_SEARCH_URL`; the repo-side internal context and RAG endpoints perform full-text/vector retrieval. The repo previously had no `help_guide` documents. The implementation now seeds 14 Help Center guide/reference documents on production startup for the existing embedding worker to vectorize. Permission-boundary review remains outstanding for broad semantic endpoints and should be completed before claiming unrestricted staff-wide retrieval.
