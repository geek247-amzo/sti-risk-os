...
The data-integrity decision is finalized: use real code paths, not a parallel sandbox, but mark all records created by the walkthrough with `is_tutorial = true`. **G6 remains in progress; the earlier completion claim was corrected after acceptance-criteria review.** Commits `7c860ff` and `50dfbc3` provide migration 044, tutorial flags on the five transaction tables, route-aware steps, real-click advancement, tutorial cookie state, core list/KPI/report exclusions, and an authenticated cleanup endpoint. The current follow-up build adds real action targets for PO capture/save, field-work creation/save, inspection start, report generation, and invoice creation, plus a production server sweep every 15 minutes that deletes tutorial records older than two hours. Build passes. Remaining acceptance work is live end-to-end manual verification, including completing the required inspection/signature path and confirming no tutorial records survive abandonment or appear in excluded views. Do not close G6 until that verification passes.
