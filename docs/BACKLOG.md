...
- E3 — **transcription complete for all available recordings**, implementation commit `5f2015b`. Recordings are downloaded to the persistent uploads volume and sent to Gemini 2.5 Flash with retryable transcription status. The authenticated `/staff/voice` panel exposes imported CDRs, recording metadata, transcript text, and transcription status. Live verification now shows 19/19 retained recording files transcribed successfully; the remaining 29 imported rows are CDR-only calls with no recording attached, not stuck retries. Customer matching/tagging and wider customer/staff views remain E4–E7.
- E4 — **build unblocked; policies finalized**. Contacts currently have one free-form `phone` field; the repo has no general CRM phone-normalization helper, no `app_users` extension field, and no staff-extension mapping table. Final policy: normalize formatting and South African country-code variants but never similarity-match; only the staff member associated with the call may tag it by default; personal rules persist per exact normalized number; personal audio is deleted after transcription while metadata and the Personal label remain. Extension ownership will be populated from Yeastar’s extension list where possible, with explicit staff setup for any unmatched extension.
...
**G2 status:** **complete and live**, commit `46651b4`. Authored walkthroughs now cover KPI drill-down, finance/cash-flow alerts, capability checklist, partner development, time tracking, testimonial/referral triggers, inspections, and management reports/exports. Yeastar call-handling content remains deferred until E3/E4 provide a verified staff workflow. Build passed; `/health` and `/ready` return 200; `/staff/help` remains protected by the staff login redirect. No new schema.
...
The data-integrity decision is finalized: use real code paths, not a parallel sandbox, but mark all records created by the walkthrough with `is_tutorial = true`. Discovery confirmed quotes, client POs, sales orders, work items, and invoices do not currently have this flag; a single additive migration can add `boolean NOT NULL DEFAULT false` to all five without changing relationships. The implementation must thread the exclusion through KPI/report queries and client-facing record views, and cleanup must be idempotent and safe if the user abandons the walkthrough. G6 is now buildable; no implementation has started.
