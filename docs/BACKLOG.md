...
- E4 — **discovery complete; build awaiting matching policy decisions**. Contacts currently have one free-form `phone` field; the repo has no general CRM phone-normalization helper, no `app_users` extension field, and no staff-extension mapping table. The implementation should add normalized comparison at the call boundary and a durable call-tag/rule model, but must first confirm: exact-only versus formatted/fuzzy number matching, who may tag calls, whether personal rules apply per number or broader context, and whether personal audio is retained after transcription.
