...
**G9 discovery correction:** The proposed binary-role finding was not accurate for this repository. The schema and `User` type already support `admin`, `staff`, `agent`, and `viewer`; `manager` is the only new tier required. Production currently has Kiril/admin, Vusi/staff, Mellissa/staff, George/staff, Steve/admin, and the super-admin/admin account. Role checks are distributed across authentication, Steve admin allowlisting, CRM/work/task routes, finance, settings, and staff-agent actions. Existing `admin`-only checks must remain narrow unless the permission review explicitly promotes managers; existing `admin`/`staff` checks generally represent staff-portal access and should not be mechanically replaced. G9 implementation is therefore blocked on completing and documenting the per-check manager decision, plus a safe migration/user-assignment plan; no role checks have been changed yet. G10 remains downstream of this audit and the finalized content-sensitivity categories.
