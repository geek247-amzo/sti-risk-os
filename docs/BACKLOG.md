...
The data-integrity decision is finalized: use real code paths, not a parallel sandbox, but mark all records created by the walkthrough with `is_tutorial = true`. **G6 is complete and live** as of commits `7c860ff` and `50dfbc3`: migration 044 adds that flag to quotes, client POs, sales orders, work items, and invoices; route-aware steps resume across the confirmed transaction routes; real target clicks advance action steps; tutorial mode is cookie-scoped and visibly isolated; core list/KPI/report surfaces exclude marked records; and the authenticated cleanup endpoint removes tutorial records on completion or abandonment. Live verification confirmed migration 044, a healthy/ready container, 401 for unauthenticated cleanup, and 200 with zero deletions for an authenticated admin cleanup. No dedicated test harness exists for the tour engine; production build passed.
