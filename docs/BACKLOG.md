...
The data-integrity decision is finalized: use real code paths, not a parallel sandbox, but mark all records created by the walkthrough with `is_tutorial = true`. Migration 044 now adds that flag to quotes, client POs, sales orders, work items, and invoices. The guided engine is in progress: route-aware steps, real-click advancement, tutorial cookie state, and quote-to-report route scaffolding are implemented; core list/KPI/report surfaces exclude marked records. Remaining work is to complete the downstream hand-off targets and idempotent cleanup of an abandoned tutorial chain before closing G6.
