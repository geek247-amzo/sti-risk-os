...
import { AlertTriangle, Check, FileDown, FileText, Loader2, Printer, RefreshCw, ShieldAlert } from "lucide-react";
...
function ReportView({ report, onBack }: { report: Report; onBack: () => void }) {
  ...
  return <div className="space-y-5 print-page"><style>{`@media print { .no-print { display:none !important } .print-page { max-width:none !important; padding:0 !important } .print-card { border:none !important; box-shadow:none !important } }`}</style><div className="no-print flex flex-wrap gap-2"><button className={`${button} border border-border bg-surface`} onClick={onBack}>← Back to reports</button><button className={`${button} bg-brand-orange text-primary-foreground`} onClick={() => window.print()}><Printer className="h-4 w-4" /><FileDown className="h-4 w-4" /> Print / Save PDF</button></div><header className="print-card rounded-xl border border-border/60 bg-surface p-5 shadow-sm md:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Site survey report · {report.status}</div><h2 className="mt-1 text-2xl font-bold">{report.title}</h2><p className="mt-2 text-sm text-muted-foreground">{report.organization?.name ?? "Customer"} · {report.site?.name ?? "Site"}</p></div><div className="flex gap-2"><StatusBadge value={report.overallOutcome} /><StatusBadge value={`${report.overallRiskLevel} risk`} risk /></div></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><Summary label="Inspection blocks" value={report.inspectionCount} /><Summary label="Overall outcome" value={report.overallOutcome} /><Summary label="Overall risk" value={report.overallRiskLevel} /></div><p className="mt-4 text-xs text-muted-foreground">Generated {new Date(report.generatedAt).toLocaleString()}</p></header>{Object.entries(groups).map(([group, blocks]) => <section key={group} className="space-y-3"><h3 className="border-b border-border pb-2 text-lg font-bold">{group}</h3>{blocks.map((block) => <InspectionBlock key={block.inspectionId} block={block} />)}</section>)}</div>;
}
